...
for model in aircraft_models:
    ...
    for ext in extensions:
        pattern = f"imgs/{safe}__*.{ext}"
        files.extend(glob.glob(pattern))

    # Sort by suffix (__a, __b, __c...) regardless of extension
    files.sort(key=lambda f: os.path.splitext(f)[0].split('__')[-1])
...
def scale_preserve_aspect(img, max_width, max_height):
    ...
    # Compute scale factor to fit within the screen
...
def scale_by_orientation(img):
    sw, sh = SCREEN_WIDTH, SCREEN_HEIGHT
    w, h = img.get_size()

    if h > w:
        # Portrait → match height
        scale = sh / h
    else:
        # Landscape → match width
        scale = sw / w
...
def scale_vacr(img):
    ...
    # First pass: orientation-based scale
    if h > w:
        # Portrait → match height
        scale = sh / h
    else:
        # Landscape or square → match width
        scale = sw / w

    new_w = int(w * scale)
    new_h = int(h * scale)

    # Second pass: clamp to screen bounds (safety)
    if new_w > sw:
        scale = sw / new_w
